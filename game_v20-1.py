...
pygame.display.set_caption("Data-Driven 3v3 Soccer Simulator (v20.1 - UI Polish)")
...
# UI COLORS (New)
STAT_ELITE = (57, 255, 20)    # Neon "Mountain Dew" Green (18-20)
STAT_GOOD = (50, 205, 50)     # Standard Lime Green (15-17)
STAT_AVG = (255, 215, 0)      # Gold/Yellow (10-14)
STAT_POOR = (255, 140, 0)     # Orange (5-9)
STAT_BAD = (220, 20, 60)      # Red (1-4)
STAT_BG = (40, 40, 60)        # Dark Blue-Grey Background
...
class Player:
    ...
    def draw(self, screen):
        ...
        # Trait Icons (NEW: Shapes instead of just circles)
        pip_x = self.rect.centerx - ((len(self.traits) * 12) // 2); pip_y = self.rect.top - 18
        for trait in self.traits:
            c = TRAIT_COLORS.get(trait, WHITE)
            if trait == "SPEED_DEMON": # Triangle
                pygame.draw.polygon(screen, c, [(pip_x, pip_y-4), (pip_x-4, pip_y+4), (pip_x+4, pip_y+4)])
            elif trait == "BULLDOZER": # Square
                pygame.draw.rect(screen, c, (pip_x-4, pip_y-4, 8, 8))
            elif trait == "SNIPER": # Crosshair
                pygame.draw.circle(screen, c, (pip_x, pip_y), 5, 1)
                pygame.draw.circle(screen, c, (pip_x, pip_y), 2)
            elif trait == "MAGNET": # Diamond
                pygame.draw.polygon(screen, c, [(pip_x, pip_y-5), (pip_x+5, pip_y), (pip_x, pip_y+5), (pip_x-5, pip_y)])
            elif trait == "INTERCEPTOR": # Shield/Pentagon
                pygame.draw.polygon(screen, c, [(pip_x-4, pip_y-2), (pip_x+4, pip_y-2), (pip_x+4, pip_y+2), (pip_x, pip_y+5), (pip_x-4, pip_y+2)])
            else:
                pygame.draw.circle(screen, c, (pip_x, pip_y), 4)
            pip_x += 12
...
def draw_hud(screen, player, score, timer, team_names, is_golden_goal=False): 
    ...
    # Removed old stamina bar from HUD since it's now floating

def draw_screen_title(screen, title): title_font = get_font(50, True); title_surf = title_font.render(title, True, WHITE); screen.blit(title_surf, (WIDTH // 2 - title_surf.get_width() // 2, 60))
...
def reset_game(player_team_name, ai_team_name, selected_players, is_golden_goal=False):
    ...
    game_objects['goal_data'] = {'scorer': None, 'timer': 0} # NEW for Celebration
    return game_objects
...
# --- UPDATED STATE HANDLERS WITH BACK BUTTON ---
...
def handle_team_selection_state(events, mouse_pos, team_buttons, scroll_y, back_button, is_tournament=False, is_golden_goal=False, is_season=False):
    ...
    for event in events:
        if back_button.is_clicked(event):
            return "MENU", None, 0 # Return to Menu
    ...
    # Draw Back Button (Fixed position, no scroll)
...
def handle_player_selection_state(events, mouse_pos, user_team, player_cards, selected_players, start_match_button, back_button, scroll_y, is_tournament=False, tournament=None, is_golden_goal=False, is_season=False, season=None):
    ...
    for event in events:
        if back_button.is_clicked(event):
            # Return to the specific team selection mode user was in
...
def handle_tournament_bracket_state(events, mouse_pos, tournament, selected_players, play_match_button, back_button):
    # Check Back Button (only if not eliminated/finished to prevent quitting mid-tournament accidentally)
    for event in events:
         if back_button.is_clicked(event):
             return "MENU", None
...
def handle_playing_state(events, keys, game_objects, active_idx, elapsed_time):
    ...
    if goal_scored:
        if is_golden_goal:
            ...
            game_objects['goal_data']['timer'] = 120 # 2 seconds
...
def main():
    ...
    # NEW: Back Button (Top Left Corner)
