...
try:
    ...
    data = []
    teams = ['lafc', 'la_galaxy', 'inter_miami', 'columbus_crew', 'cincinnati']
    for t in teams:
        for i in range(5):
            data.append({'Player': f'{t}_{i}', 'Team': t, 'Position': 'FW', 'Pace': 12, 'Shooting': 12, 'Passing': 12, 'Dribbling': 12, 'Defense': 12})
    player_profiles_df = pd.DataFrame(data)
...
class Tournament:
    ...
    def draw_bracket(self, screen):
        ...
        pygame.draw.line(screen, WHITE, (300, 200), (300, 400), 2) 
        pygame.draw.line(screen, WHITE, (300, 300), (400, 300), 2) 
        ...
        pygame.draw.line(screen, WHITE, (700, 200), (700, 400), 2)
        pygame.draw.line(screen, WHITE, (700, 300), (600, 300), 2)
    ...
    def update(self, ball, player_team, ai_team, is_presser, game_stats):
        ...
        if ball.owner == self:
            ...
            if action == "shoot" and dist_to_goal < 400:
                ...
                teammate = find_best_pass_target(self, ai_team, player_team); 
...
def draw_hud(screen, player, score, timer, team_names, is_golden_goal=False): 
    font = pygame.font.SysFont(None, 40)
    ...
    # Timer Display logic
    if is_golden_goal:
        timer_text = font.render("GOLDEN GOAL", True, GOLD)
        screen.blit(timer_text, (WIDTH-220, 10))
    else:
...
def find_best_pass_target(player, team, opponent_team):
    ...
    for teammate in team:
        ...
        score = (100 / distance) + (forwardness * 0.1)
        blocked = False
        for opponent in opponent_team:
            if opponent.rect.inflate(10, 10).clipline(player.x, player.y, teammate.x, teammate.y):
                blocked = True
                break
        if blocked: score -= 50
...
def draw_stats_screen(screen, game_objects):
    ...
    # --- GOLDEN GOAL TITLE ---
    if game_objects.get('is_golden_goal') and game_objects.get('won_by_golden_goal'):
        draw_screen_title(screen, "GOLDEN GOAL VICTORY!")
    else:
        draw_screen_title(screen, "Match Statistics")
...
def handle_menu_state(events, mouse_pos, menu_buttons, tournament):
    for event in events:
        ...
        if menu_buttons['golden_goal'].is_clicked(event): return "TEAM_SELECTION_GOLDEN", None # New Button
...
def handle_team_selection_state(events, mouse_pos, team_buttons, is_tournament=False, is_golden_goal=False):
    for event in events:
    ...
    if is_golden_goal: title_text += " (Golden Goal Mode)"
    
    screen.fill(GREEN); draw_screen_title(screen, title_text)
    for button in team_buttons.values(): button.check_hover(mouse_pos); button.draw(screen)
    
    # Return same state logic, checking tournament later
    if is_tournament: return "TEAM_SELECTION_TOURNAMENT", None
    if is_golden_goal: return "TEAM_SELECTION_GOLDEN", None
    return "TEAM_SELECTION", None

def handle_player_selection_state(events, mouse_pos, user_team, player_cards, selected_players, start_match_button, is_tournament=False, tournament=None, is_golden_goal=False):
    for event in events:
    ...
    for event in events:
        if tournament.current_round == "SEMI" or tournament.current_round == "FINAL":
             if tournament.user_status == "ALIVE" and play_match_button.is_clicked(event):
                ...
                game_objects = reset_game(tournament.user_team, opponent, selected_players, is_golden_goal=False) # Tournament is normal time
                if game_objects: whistle_sound.play(); return "PLAYING", game_objects
...
def handle_penalty_shootout_state(events, keys, game_objects, penalty_data):
    ...
    if 'anim_state' not in penalty_data:
        penalty_data['anim_state'] = 'aiming' 
        penalty_data['kicker_pos'] = KICKER_START_POS.copy()
        penalty_data['ball_pos'] = BALL_START_POS.copy()
        penalty_data['gk_pos'] = GK_START_POS.copy()
        penalty_data['target_ball'] = None
        penalty_data['target_gk'] = None
        penalty_data['gk_feint_offset'] = 0
        penalty_data['has_hesitated'] = False
        penalty_data['hesitation_end'] = 0
    ...
    if penalty_data['anim_state'] == 'result_display':
        if current_time > penalty_data['timer']:
            ...
            if penalty_data['phase'] == 'USER_KICK':
                penalty_data['phase'] = 'AI_KICK'
                penalty_data['charge'] = 0
                penalty_data['user_dir'] = None
            else:
                penalty_data['phase'] = 'USER_KICK'
                penalty_data['round'] += 1
                u_score = penalty_data['user_history'].count('O')
                a_score = penalty_data['ai_history'].count('O')
                rounds_played = penalty_data['round'] - 1
                if rounds_played < 3:
            ...
            color = GOLD if "GOAL" in penalty_data['result_msg'] else RED
            msg = font.render(penalty_data['result_msg'], True, color)
            ...
            if penalty_data['user_dir'] is None:
                ...
                for event in events:
                    if event.type == pygame.KEYUP and event.key == pygame.K_SPACE:
                        ai_choice = random.choice(["LEFT", "CENTER", "RIGHT"])
                        penalty_data['ai_move'] = ai_choice
                        
                        if penalty_data['charge'] > 95:
                            penalty_data['shot_result'] = "MISS"
                            penalty_data['result_msg'] = "MISS! (Overpowered)"
                        elif penalty_data['user_dir'] == ai_choice:
                            penalty_data['shot_result'] = "SAVE"
                            penalty_data['result_msg'] = "SAVED!"
                        else:
                            penalty_data['shot_result'] = "GOAL"
                            penalty_data['result_msg'] = "GOAL!"
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in [pygame.K_LEFT, pygame.K_UP, pygame.K_RIGHT]:
                        ...
                        if random.random() < 0.1:
                             penalty_data['shot_result'] = "MISS"
                             penalty_data['result_msg'] = "MISS! (AI Wide)"
                        elif user_dive == ai_shot:
                             penalty_data['shot_result'] = "SAVE"
                             penalty_data['result_msg'] = "SAVED!"
                        else:
                             penalty_data['shot_result'] = "GOAL"
    ...
    if penalty_data['anim_state'] == 'running':
        if penalty_data['phase'] == 'AI_KICK' and not penalty_data['has_hesitated']:
            ...
            if dist < 80 and dist > 40 and random.random() < 0.5:
                penalty_data['anim_state'] = 'hesitating'
                penalty_data['hesitation_end'] = current_time + random.randint(200, 500) 
                penalty_data['has_hesitated'] = True

        direction = penalty_data['ball_pos'] - penalty_data['kicker_pos']
        if direction.length() > 5:
            direction.normalize_ip()
            penalty_data['kicker_pos'] += direction * 4
        else:
            penalty_data['anim_state'] = 'kicking'
            kick_sound.play()

    elif penalty_data['anim_state'] == 'hesitating':
        if current_time > penalty_data['hesitation_end']:
            penalty_data['anim_state'] = 'running'

    elif penalty_data['anim_state'] == 'kicking':
        ball_dir = penalty_data['target_ball'] - penalty_data['ball_pos']
        gk_dir = penalty_data['target_gk'] - penalty_data['gk_pos']
        
        if ball_dir.length() > 10:
            ball_dir.normalize_ip()
            penalty_data['ball_pos'] += ball_dir * 15 
        
        if gk_dir.length() > 5:
            gk_dir.normalize_ip()
            penalty_data['gk_pos'] += gk_dir * 8 
            
        if penalty_data['ball_pos'].distance_to(penalty_data['target_ball']) < 15:
            penalty_data['anim_state'] = 'result_display'
            penalty_data['timer'] = current_time + 2000 
            if penalty_data['shot_result'] == "GOAL": goal_sound.play()
            elif penalty_data['shot_result'] == "SAVE": tackle_sound.play()

    screen.fill(DARK_GREEN)
    pygame.draw.rect(screen, WHITE, (WIDTH//2 - 150, GOAL_TOP_Y, 300, 10)) 
    pygame.draw.rect(screen, WHITE, (WIDTH//2 - 150, GOAL_TOP_Y, 10, 150)) 
    pygame.draw.rect(screen, WHITE, (WIDTH//2 + 150, GOAL_TOP_Y, 10, 150)) 
    for i in range(1, 10):
        pygame.draw.line(screen, (200, 200, 200), (WIDTH//2 - 150 + i*30, GOAL_TOP_Y), (WIDTH//2 - 150 + i*30, GOAL_TOP_Y+150), 1)
        pygame.draw.line(screen, (200, 200, 200), (WIDTH//2 - 150, GOAL_TOP_Y + i*15), (WIDTH//2 + 150, GOAL_TOP_Y + i*15), 1)

    gk_color = GOLD if penalty_data['phase'] == 'USER_KICK' else TEAM_COLORS['default_player']
    pygame.draw.circle(screen, gk_color, (int(penalty_data['gk_pos'].x), int(penalty_data['gk_pos'].y)), 20)
    kicker_color = TEAM_COLORS['default_player'] if penalty_data['phase'] == 'USER_KICK' else TEAM_COLORS['default_ai']
    pygame.draw.circle(screen, kicker_color, (int(penalty_data['kicker_pos'].x), int(penalty_data['kicker_pos'].y)), 20)
    ...
    if penalty_data['anim_state'] == 'aiming':
        ...
        if penalty_data['phase'] == 'USER_KICK':
            if penalty_data['user_dir'] is None: msg = "Aim: Arrow Keys"
                ...
                msg = "Hold SPACE (Power)"
                bar_w = 200; bar_h = 20
                pygame.draw.rect(screen, BLACK, (WIDTH//2 - bar_w//2, HEIGHT - 80, bar_w, bar_h))
                fill = (penalty_data['charge'] / 100) * bar_w
                c = GREEN if penalty_data['charge'] < 60 else (YELLOW if penalty_data['charge'] < 95 else RED)
                pygame.draw.rect(screen, c, (WIDTH//2 - bar_w//2, HEIGHT - 80, fill, bar_h))
        else:
            msg = "Predict Dive: Arrow Keys"
    ...
    start_x = WIDTH//2 - 150
    for i in range(5):
        res = penalty_data['user_history'][i] if i < len(penalty_data['user_history']) else None
        color = GREEN if res == 'O' else (RED if res == 'X' else GRAY)
        pygame.draw.circle(screen, color, (start_x + i*60, 30), 15)
        
        res_a = penalty_data['ai_history'][i] if i < len(penalty_data['ai_history']) else None
        color_a = GREEN if res_a == 'O' else (RED if res_a == 'X' else GRAY)
        pygame.draw.circle(screen, color_a, (start_x + i*60, 70), 15)
        
    font = pygame.font.SysFont(None, 30)
    screen.blit(font.render("YOU", True, WHITE), (start_x - 50, 20))
    screen.blit(font.render("CPU", True, WHITE), (start_x - 50, 60))
...
def handle_playing_state(events, keys, game_objects, active_idx, elapsed_time):
    ...
    active_player.move(keys, ball.owner == active_player)
    ...
    if goal_scored:
        if is_golden_goal:
            game_objects['won_by_golden_goal'] = True
            whistle_sound.play()
            return "POST_GAME", active_idx
        
        [p.reset() for p in player_team+ai_team]; ball.reset()

    # TIMER LOGIC
    timer = GAME_DURATION_SECONDS - ((pygame.time.get_ticks()-game_objects['start_ticks'])/1000)
    
    # Check for game end ONLY if NOT golden goal mode
    if not is_golden_goal and timer <= 0: 
        whistle_sound.play()
        return "POST_GAME", active_idx
    ...
    draw_hud(screen, active_player, game_objects['score'], timer, game_objects['team_names'], is_golden_goal)
    return "PLAYING", active_idx


def handle_post_game_state(events, game_objects, is_tournament, tournament, penalty_data):
    for event in events:
    ...
    if is_tournament and game_objects['score']['Player'] == game_objects['score']['AI']:
        font = pygame.font.SysFont(None, 40)
        text = font.render("DRAW! Press Enter for Penalty Shootout", True, GOLD)
        screen.blit(text, (WIDTH//2 - text.get_width()//2, HEIGHT - 130))
...
def main():
    ...
    menu_buttons = {
        "start": Button(WIDTH//2-100, 250, 200, 60, "Friendly", BLUE, (0,100,255)),
        "golden_goal": Button(WIDTH//2-100, 330, 200, 60, "Golden Goal", GOLD, (255,215,0)), # New Button
        "mls_cup": Button(WIDTH//2-100, 410, 200, 60, "MLS Cup", ORANGE, (255,200,0)), 
        "quit": Button(WIDTH//2-100, 490, 200, 60, "Quit", RED, (255,100,100))
    }
    ...
    is_golden_goal_mode = False # New Flag
    penalty_data = {} 
    ...
    while running:
        ...
        if game_state == "MENU":
            ...
            elif next_state == "TEAM_SELECTION": 
                game_state = "TEAM_SELECTION"
                is_tournament_mode = False
                is_golden_goal_mode = False
            elif next_state == "TEAM_SELECTION_TOURNAMENT": 
                game_state = "TEAM_SELECTION"
                is_tournament_mode = True
                is_golden_goal_mode = False
            elif next_state == "TEAM_SELECTION_GOLDEN": # New State
                game_state = "TEAM_SELECTION"
                is_tournament_mode = False
                is_golden_goal_mode = True
            
        elif game_state == "TEAM_SELECTION":
            next_state, team_choice = handle_team_selection_state(events, mouse_pos, team_buttons, is_tournament_mode, is_golden_goal_mode)
            if team_choice:
            ...
            next_state, result = handle_player_selection_state(events, mouse_pos, user_team, player_cards, selected_players, start_match_button, is_tournament_mode, tournament, is_golden_goal_mode)
            ...
            game_state = handle_post_game_state(events, game_objects, is_tournament_mode, tournament, penalty_data)
            ...
            if result == "POST_GAME_PENALTY_DONE":
                user_won = penalty_data['user_history'].count('O') > penalty_data['ai_history'].count('O')
                tournament.advance_tournament(user_won)
                game_state = "TOURNAMENT_BRACKET"
