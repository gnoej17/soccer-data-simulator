...
pygame.display.set_caption("Data-Driven 3v3 Soccer Simulator (v19 - Build-Up Play Update)")
...
# TRAIT VISUAL CONFIG
TRAIT_COLORS = {
    "SPEED_DEMON": (0, 255, 255),  # Cyan
    "BULLDOZER": (139, 69, 19),    # Brown
    "SNIPER": (255, 0, 0),         # Red
    "MAGNET": (255, 0, 255),       # Magenta
    "INTERCEPTOR": (255, 255, 0)   # Yellow
}
...
class Player:
    def __init__(self, x, y, team_name, name, stats_dict):
        ...
        # New Field: Pass Cooldown (Prevents instant ping-pong passing)
        self.pass_cooldown = 0

        # --- TRAIT SYSTEM INITIALIZATION ---
        self.traits = []
        if self.weight_kg > 80: self.traits.append("BULLDOZER")         # Harder to be tackled
        if self.stats.get('Defense', 0) >= 15 and self.height_cm > 182: self.traits.append("INTERCEPTOR") # Larger reach
        if self.pace >= 15: self.traits.append("SPEED_DEMON")           # Less sprint stamina
        if self.stats.get('Shooting', 0) >= 15: self.traits.append("SNIPER") # Faster shots
        if self.stats.get('Dribbling', 0) >= 16: self.traits.append("MAGNET") # Better ball control
    ...
    def move(self, keys, ball_owner):
        ...
        # --- TRAIT: SPEED DEMON (Reduced Stamina Cost) ---
    ...
    def draw(self, screen):
        ...
        # --- TRAIT VISUALIZATION ---
        pip_x = self.rect.centerx - ((len(self.traits) * 6) // 2); pip_y = self.rect.top - 15
        for trait in self.traits:
            if trait in TRAIT_COLORS:
                pygame.draw.circle(screen, TRAIT_COLORS[trait], (pip_x, pip_y), 4)
                pip_x += 8
    ...
    def tackle(self, ball, opponent_team):
        ...
        # --- TRAIT: INTERCEPTOR (Longer Reach) ---
        ...
        if target_player in opponent_team and self.get_pos().distance_to(target_player.get_pos()) < effective_radius:
            ...
            # --- TRAIT: BULLDOZER (Resistance) ---
            if "BULLDOZER" in target_player.traits:
                tackle_success_prob *= 0.6 # Harder to tackle
    ...
    def update_ai_movement(self, ball, team_has_ball, target_override=None, sprint_override=False):
        speed_multiplier = 1.0
        ...
        if direction.length() > 20:
            ...
            # --- NERFED AGGRESSIVE DEFENDING UPDATE ---
            if sprint_override:
                calculated_speed *= 1.25 # Reduced from 1.4 to 1.25 (25% boost)
...
class AIPlayer(Player):
    def update(self, ball, player_team, ai_team, is_presser, game_stats):
        ...
        if ball.owner == self:
            ...
            # 2. Dynamic Weights based on Field Position
            w_shoot = self.stats.get('Shooting', 10)
            w_pass = self.stats.get('Passing', 10)
            w_dribble = self.stats.get('Dribbling', 10)
            
            # FORCE DRIBBLE IF COOLDOWN ACTIVE (Prevents Ping-Pong)
            if self.pass_cooldown > 0 and dist_to_goal > 250:
                w_pass = 0
                w_shoot = 0
                w_dribble = 100

            if dist_to_goal < 250:
                # DANGER ZONE: High chance to shoot
                w_shoot *= 10
                w_pass *= 0.1
                w_dribble *= 0.5
            elif dist_to_goal < 450:
                # ATTACKING THIRD
                w_shoot *= 2
                w_dribble *= 2.5 
                w_pass *= 1.2
            ...
            if action == "shoot" and dist_to_goal < 400:
                ...
                # FIXED: Pass ball.last_toucher to exclude them from target list
                ...
                if teammate: ball.shoot(angle, BALL_SPEED * 0.8); return
                    ...
                    # If no good pass target found, force dribble
                    self.update_ai_movement(ball, True, goal_center)
            
            # 4. Execute Dribble (Force Movement Towards Goal)
            self.update_ai_movement(ball, True, goal_center)

        else:
            # --- DEFENDING LOGIC ---
            if is_presser: 
                # SPRINT TO BALL (Aggressive Pressing)
                self.update_ai_movement(ball, False, pygame.Vector2(ball.x, ball.y), sprint_override=True); 
                self.tackle(ball, player_team)
            else: 
                self.update_ai_movement(ball, False)
...
def find_best_pass_target(player, team, opponent_team, exclude_player=None):
    ...
    for teammate in team:
        ...
        if exclude_player and teammate == exclude_player: continue # "No Backsies" Rule

        distance = player.get_pos().distance_to(teammate.get_pos())
        if distance == 0: continue
        forwardness = (teammate.x - player.x) * player.attack_direction 
        score = (100 / distance) + (forwardness * 0.5) # Increased Forward Weight
        blocked = False
...
def handle_penalty_shootout_state(events, keys, game_objects, penalty_data):
    ...
    if penalty_data['anim_state'] == 'result_display':
        if current_time > penalty_data['timer']:
            ...
            font = pygame.font.SysFont(None, 80); color = GOLD if "GOAL" in penalty_data['result_msg'] else RED; msg = font.render(penalty_data['result_msg'], True, color)
    ...
    if penalty_data['anim_state'] == 'result_display':
        font = pygame.font.SysFont(None, 80); color = GOLD if "GOAL" in penalty_data['result_msg'] else RED; msg = font.render(penalty_data['result_msg'], True, color); screen.blit(msg, (WIDTH//2 - msg.get_width()//2, HEIGHT//2))
    ...
    font = pygame.font.SysFont(None, 30); screen.blit(font.render("YOU", True, WHITE), (start_x - 50, 20)); screen.blit(font.render("CPU", True, WHITE), (start_x - 50, 60))
...
def handle_playing_state(events, keys, game_objects, active_idx, elapsed_time):
    ...
    for event in events:
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE and ball.owner == active_player:
                ...
                # --- TRAIT: SNIPER (Faster User Shots) ---
    ...
    for p in player_team + ai_team:
        if ball.owner is None and p.rect.colliderect(ball.rect) and (p != ball.last_toucher or current_time - ball.last_touch_time > 200):
            ball.owner = p
            # SET COOLDOWN ON RECEIPT (55 frames = 0.85 seconds of forced dribbling)
            p.pass_cooldown = 55 
...
def handle_post_game_state(events, game_objects, is_tournament, tournament, penalty_data):
    ...
    if is_tournament and game_objects['score']['Player'] == game_objects['score']['AI']:
        font = pygame.font.SysFont(None, 40); text = font.render("DRAW! Press Enter for Penalty Shootout", True, GOLD); screen.blit(text, (WIDTH//2 - text.get_width()//2, HEIGHT - 130))
    return "POST_GAME"
...
def main():
    ...
    menu_buttons = {
        ...
        "golden_goal": Button(WIDTH//2-100, 330, 200, 60, "Golden Goal", GOLD, (255,215,0)), 
    ...
    tournament = None; is_tournament_mode = False; is_golden_goal_mode = False; penalty_data = {} 
    ...
    while running:
        ...
        if game_state == "MENU":
            ...
            elif next_state == "TEAM_SELECTION": game_state = "TEAM_SELECTION"; is_tournament_mode = False; is_golden_goal_mode = False
            elif next_state == "TEAM_SELECTION_TOURNAMENT": game_state = "TEAM_SELECTION"; is_tournament_mode = True; is_golden_goal_mode = False
            elif next_state == "TEAM_SELECTION_GOLDEN": game_state = "TEAM_SELECTION"; is_tournament_mode = False; is_golden_goal_mode = True
            ...
            if next_state == "PLAYING": game_objects, game_state, active_idx = result, "PLAYING", 0
        ...
        elif game_state == "PLAYING": game_state, active_idx = handle_playing_state(events, keys, game_objects, active_idx, elapsed_time)
        elif game_state == "POST_GAME": game_state = handle_post_game_state(events, game_objects, is_tournament_mode, tournament, penalty_data)
