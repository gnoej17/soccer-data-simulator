...
pygame.display.set_caption("Data-Driven 3v3 Soccer Simulator (v13 - Logic Fixed)")
...
TEAM_COLORS = {
    'lafc': (0, 0, 0),             # Black
    'la_galaxy': (255, 255, 255),  # White (Changed for visibility vs Black)
...
class Player:
    def __init__(self, x, y, team_name, name, stats_dict):
        ...
        if 'ai' in team_name: 
             self.color = TEAM_COLORS.get(team_name, TEAM_COLORS['default_ai'])
        else:
             self.color = TEAM_COLORS.get(team_name, TEAM_COLORS['default_player'])
...
class AIPlayer(Player):
    def update(self, ball, player_team, ai_team, is_presser, game_stats):
        ...
        if ball.owner == self:
            ...
            weights = {"shoot": self.stats.get('Shooting',10), "pass": self.stats.get('Passing',10), "dribble": self.stats.get('Dribbling',10)}
...
class Ball:
    def __init__(self, x, y):
        ...
        self.last_toucher, self.last_touch_time, self.last_toucher_team = None, 0, None # Team Name
        self.spin_effect = 0
    ...
    def shoot(self, angle, speed, spin_effect=0):
        kick_sound.play()
        if self.owner:
            self.last_toucher = self.owner
            self.last_touch_time = pygame.time.get_ticks()
            self.last_toucher_team = self.owner.team_name
...
def draw_hud(screen, player, score, timer, team_names, user_team_name=None): 
    ...
    p_color = TEAM_COLORS.get(team_names['player_code'], WHITE)
    ai_color = TEAM_COLORS.get(team_names['ai_code'], WHITE)
...
def draw_stats_screen(screen, game_objects):
    ...
    player_header = font_header.render(teams['player'], True, (100, 100, 255)); ai_header = font_header.render(teams['ai'], True, (255, 100, 100))
    ...
    for i, stat_name in enumerate(stat_list):
        ...
        player_stat_surf = font_stat.render(str(player_stats[i]), True, WHITE); screen.blit(player_stat_surf, (WIDTH*0.25-player_stat_surf.get_width()//2, stat_y + i*50))
        ai_stat_surf = font_stat.render(str(ai_stats[i]), True, WHITE); screen.blit(ai_stat_surf, (WIDTH*0.75-ai_stat_surf.get_width()//2, stat_y + i*50))
...
def reset_game(player_team_name, ai_team_name, selected_players):
    ...
    # FIX: Do not overwrite 'ai_team' (which is the list). Use a new key 'ai_team_name'.
    game_objects['ai_team_name'] = ai_team_name 
    return game_objects

# --- GAME STATE HANDLERS ---
def handle_menu_state(events, mouse_pos, menu_buttons):
    for event in events:
...
def handle_team_selection_state(events, mouse_pos, team_buttons):
    for event in events:
        for team_name, button in team_buttons.items():
            if button.is_clicked(event): return "PLAYER_SELECTION", team_name
    screen.fill(GREEN); draw_screen_title(screen, "Choose Your Team")
    for button in team_buttons.values(): button.check_hover(mouse_pos); button.draw(screen)
    return "TEAM_SELECTION", None

def handle_player_selection_state(events, mouse_pos, user_team, player_cards, selected_players, start_match_button):
    for event in events:
...
def handle_playing_state(events, keys, game_objects, active_idx, elapsed_time):
    ...
    for p in player_team + ai_team:
        ...
        if ball.owner is None and p.rect.colliderect(ball.rect) and (p != ball.last_toucher or current_time - ball.last_touch_time > 200):
            ...
            if ball.last_toucher and ball.last_toucher_team == p.team_name and current_time - ball.last_touch_time < PASS_COMPLETION_TIME_LIMIT:
                 # Check team_name instead of color
    ...
    # <<<<<<< GOAL LOGIC FIX (Team Name Check) >>>>>>>
    if ball.rect.colliderect((WIDTH-15, GOAL_Y_START, 15, GOAL_WIDTH)): 
        if ball.last_toucher and ball.last_toucher.team_name == game_objects['user_team']: 
         ...
         # FIX: Use 'ai_team_name' for the string comparison logic
         if ball.last_toucher and ball.last_toucher.team_name == game_objects['ai_team_name']:
...
def handle_post_game_state(events, game_objects):
    for event in events:
        if event.type == pygame.KEYDOWN and (event.key == pygame.K_RETURN or event.key == pygame.K_KP_ENTER): return "MENU"
...
def main():
    ...
    menu_buttons = {"start": Button(WIDTH//2-100, 300, 200, 60, "Start Game", BLUE, (0,100,255)), "quit": Button(WIDTH//2-100, 400, 200, 60, "Quit", RED, (255,100,100))}
    ...
    while running:
        ...
        if game_state == "MENU":
            next_state, _ = handle_menu_state(events, mouse_pos, menu_buttons)
            if next_state == "QUIT": running = False
            else: game_state = next_state
        elif game_state == "TEAM_SELECTION":
            next_state, team_choice = handle_team_selection_state(events, mouse_pos, team_buttons)
            if team_choice:
                ...
                card_width, card_height, gap = 140, 160, 20 
                num_cols = 5; total_width = (card_width + gap) * num_cols - gap; start_x = (WIDTH - total_width) // 2
            ...
            next_state, new_game_objects = handle_player_selection_state(events, mouse_pos, user_team, player_cards, selected_players, start_match_button)
            if new_game_objects: game_objects, game_state, active_idx = new_game_objects, next_state, 0
        elif game_state == "PLAYING":
            game_state, active_idx = handle_playing_state(events, keys, game_objects, active_idx, elapsed_time)
        elif game_state == "POST_GAME":
            game_state = handle_post_game_state(events, game_objects)
