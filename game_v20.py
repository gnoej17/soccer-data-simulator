...
pygame.display.set_caption("Data-Driven 3v3 Soccer Simulator (v20 - league mode)")
...
TEAM_COLORS = {
    ...
    'seattle_sounders': (93, 151, 50), # olive green
    'charlotte_fc': (0, 168, 224),     # Carolina Blue
    'minnesota_united': (155, 165, 175), # Light Gray
    'san_jose_earthquakes': (0, 102, 204), # vivid blue
    'philadelphia_union': (23, 72, 155), # Union Blue
...
class Button:
    ...
    def draw(self, screen, scroll_offset=0):
        ...
        if draw_rect.bottom < 0 or draw_rect.top > HEIGHT:
            return
...
class PlayerCard:
    ...
    def draw(self, screen, is_selected, scroll_offset=0):
        ...
        for stat, value in self.stats.items():
            ...
            screen.blit(stat_surf, (draw_rect.x + 10, draw_rect.y + y_offset)); y_offset += 18
...
class Season:
    ...
    def draw_standings(self, screen):
        ...
        header_font = pygame.font.SysFont(None, 36); row_font = pygame.font.SysFont(None, 28)
        title = pygame.font.SysFont(None, 50).render(f"LEAGUE TABLE - ROUND {self.current_round_idx + 1}/{len(self.schedule)}", True, GOLD)
        if self.season_over: title = pygame.font.SysFont(None, 50).render("FINAL STANDINGS", True, GOLD)
...
class Tournament:
    ...
    def draw_bracket(self, screen):
        screen.fill(BLACK)
        font = pygame.font.SysFont(None, 40); small_font = pygame.font.SysFont(None, 30)
        title = pygame.font.SysFont(None, 60).render("MLS CUP BRACKET", True, GOLD); screen.blit(title, (WIDTH//2 - title.get_width()//2, 50))
...
class Player:
    def __init__(self, x, y, team_name, name, stats_dict):
        ...
        self.direction_vector = pygame.Vector2(0, 0); self.home_position = pygame.Vector2(x, y); self.font = pygame.font.SysFont(None, 20); self.goals = 0; self.super_armor_timer = 0
    ...
    def draw(self, screen):
        ...
        for trait in self.traits:
            if trait in TRAIT_COLORS:
                pygame.draw.circle(screen, TRAIT_COLORS[trait], (pip_x, pip_y), 4); pip_x += 8
    ...
    def update(self, ball, player_team, ai_team, is_presser, game_stats):
        ...
        if ball.owner == self:
            ...
            if action == "shoot" and dist_to_goal < 400:
                ...
                else: self.update_ai_movement(ball, True, goal_center)
            self.update_ai_movement(ball, True, goal_center)
        else:
            if is_presser: 
                self.update_ai_movement(ball, False, pygame.Vector2(ball.x, ball.y), sprint_override=True); self.tackle(ball, player_team)
...
def draw_stats_screen(screen, game_objects):
    ...
    nav_font = pygame.font.SysFont(None, 30); nav_text = nav_font.render("Press ENTER to continue", True, WHITE); screen.blit(nav_text, (WIDTH//2 - nav_text.get_width()//2, HEIGHT-80))
...
# UPDATED: Added Scroll Support for Team Selection
def handle_team_selection_state(events, mouse_pos, team_buttons, scroll_y, is_tournament=False, is_golden_goal=False, is_season=False):
    max_scroll = max(0, len(team_buttons) * 80 + 200 - HEIGHT + 50) # Calculate limit
    ...
    for event in events:
        ...
        for team_name, button in team_buttons.items():
            # Pass scroll_y to is_clicked to fix click offset bug
            if button.is_clicked(event, scroll_offset=scroll_y): 
                return "PLAYER_SELECTION", team_name, 0 # Reset scroll for next screen

    title_text = "Choose Your Team (Scroll with Mouse Wheel)"; 
    ...
    for button in team_buttons.values(): 
        button.check_hover(mouse_pos, scroll_offset=scroll_y) # Fix hover offset
        button.draw(screen, scroll_offset=scroll_y) # Fix draw offset
...
# UPDATED: Added Scroll Support for Player Selection
def handle_player_selection_state(events, mouse_pos, user_team, player_cards, selected_players, start_match_button, scroll_y, is_tournament=False, tournament=None, is_golden_goal=False, is_season=False, season=None):
    # Calculate max scroll for players
    ...
    for event in events:
        ...
        # Start button is fixed at bottom, no scroll needed for it
    ...
    screen.fill(GREEN); draw_screen_title(screen, f"Select Squad ({len(selected_players)}/3) - Scroll to see more")
...
def handle_league_table_state(events, mouse_pos, season, selected_players, play_match_button):
    for event in events:
    ...
    if not season.season_over:
        ...
        font = pygame.font.SysFont(None, 40); text = font.render(msg, True, WHITE); screen.blit(text, (WIDTH//2 - text.get_width()//2, HEIGHT - 30))
...
def handle_tournament_bracket_state(events, mouse_pos, tournament, selected_players, play_match_button):
    ...
    if tournament.user_status == "ALIVE":
        ...
        font = pygame.font.SysFont(None, 40); text = font.render(msg, True, WHITE); screen.blit(text, (WIDTH//2 - text.get_width()//2, HEIGHT - 100))
...
def main():
    ...
    # NEW: Scroll variable shared across states
    menu_scroll_y = 0
    ...
    while running:
        ...
        if game_state == "MENU":
            ...
            if next_state == "QUIT": running = False
                ...
                menu_scroll_y = 0 # Reset scroll when entering team selection
            
        elif game_state == "TEAM_SELECTION":
            # UPDATED: Pass and receive scroll value
            next_state, team_choice, menu_scroll_y = handle_team_selection_state(
                events, mouse_pos, team_buttons, menu_scroll_y, is_tournament_mode, is_golden_goal_mode, is_season_mode
            )
            if team_choice:
                user_team, game_state, selected_players = team_choice, next_state, []
                menu_scroll_y = 0 # Reset scroll for player selection
            ...
            next_state, result, menu_scroll_y = handle_player_selection_state(
                events, mouse_pos, user_team, player_cards, selected_players, start_match_button, menu_scroll_y, 
            ...
            next_state, new_game_objects = handle_league_table_state(events, mouse_pos, season, selected_players, play_match_button)
            if next_state == "PLAYING": game_objects, game_state, active_idx = new_game_objects, "PLAYING", 0
            elif next_state == "MENU": game_state = "MENU"
