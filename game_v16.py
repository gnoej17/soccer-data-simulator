...
pygame.display.set_caption("Data-Driven 3v3 Soccer Simulator (v16 - Mind Games Update)")
...
# <<<<<<< NEW: ANIMATED PENALTY SHOOTOUT WITH MIND GAMES >>>>>>>
def handle_penalty_shootout_state(events, keys, game_objects, penalty_data):
    ...
    # Initialize Animation Entities (One time only when state starts or resets)
    if 'anim_state' not in penalty_data:
        penalty_data['anim_state'] = 'aiming' # aiming, hesitating, running, kicking, result_display
    ...
    if penalty_data['anim_state'] == 'result_display':
        if current_time > penalty_data['timer']:
            ...
            # Still draw scene below
            pass 

    # --- 2. INPUT PHASE (Aiming) ---
    elif penalty_data['anim_state'] == 'aiming':
        if penalty_data['phase'] == 'USER_KICK':
            
            # <<<<<< MIND GAME: AI GK FEINT >>>>>>
            # Wiggle the GK back and forth
            penalty_data['gk_feint_offset'] = math.sin(current_time * 0.01) * 30 # +/- 30px wiggle
            ...
            if penalty_data['user_dir'] is None:
                ...
                for event in events:
                    if event.type == pygame.KEYUP and event.key == pygame.K_SPACE:
                        ...
                        # Reset GK position to center before diving (remove feint offset)
                        penalty_data['gk_pos'].x = WIDTH // 2 
                        penalty_data['anim_state'] = 'running' 

        else: # AI KICK (User is GK)
            # No feints for user GK (too complex control)
            for event in events:
    ...
    # --- 3. ANIMATION UPDATE PHASE ---
    if penalty_data['anim_state'] == 'running':
        
        # <<<<<< MIND GAME: KICKER HESITATION >>>>>>
        if penalty_data['phase'] == 'AI_KICK' and not penalty_data['has_hesitated']:
            ...
            # Hesitate when getting closer (50% chance)
            if dist < 80 and dist > 40 and random.random() < 0.5:
                penalty_data['anim_state'] = 'hesitating'
                penalty_data['hesitation_end'] = current_time + random.randint(200, 500) # 200-500ms stutter
        ...
        if ball_dir.length() > 10:
            ...
            penalty_data['ball_pos'] += ball_dir * 15 # Fast Ball
        
        if gk_dir.length() > 5:
            gk_dir.normalize_ip()
            penalty_data['gk_pos'] += gk_dir * 8 # Slower GK
        ...
        if penalty_data['ball_pos'].distance_to(penalty_data['target_ball']) < 15:
            ...
            penalty_data['timer'] = current_time + 2000 # Show result for 2s
    ...
    # Draw Goal Structure
    pygame.draw.rect(screen, WHITE, (WIDTH//2 - 150, GOAL_TOP_Y, 300, 10)) # Crossbar
    pygame.draw.rect(screen, WHITE, (WIDTH//2 - 150, GOAL_TOP_Y, 10, 150)) # Left Post
    pygame.draw.rect(screen, WHITE, (WIDTH//2 + 150, GOAL_TOP_Y, 10, 150)) # Right Post
    # Net lines
    for i in range(1, 10):
...
def handle_playing_state(events, keys, game_objects, active_idx, elapsed_time):
    # This function MUST return a tuple (state, active_idx) in ALL paths
        ...
        if event.type == pygame.KEYDOWN and (event.key == pygame.K_RETURN or event.key == pygame.K_KP_ENTER):
            if is_tournament:
                if game_objects['score']['Player'] == game_objects['score']['AI']:
                    penalty_data.clear()
                    # FIXED: Do NOT set 'anim_state' or positions here.
                    # only set the game logic variables. The handler will set up 
                    # the visuals automatically when it sees
                    penalty_data.update({
                        'phase': 'USER_KICK', 
                        'round': 1, 
                        'user_history': [], 
...
def main():
    ...
    menu_buttons = {
        ...
        "mls_cup": Button(WIDTH//2-100, 400, 200, 60, "MLS Cup", ORANGE, (255,200,0)), 
    ...
    while running:
        ...
        if game_state == "MENU":
            ...
            # Pass KEYS to this function now
